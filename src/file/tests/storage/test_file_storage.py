...
from ..mocks import ETAG, FILE_ID, NAME, RELATIVE_URL, SIZE, UPLOAD_ID, USER_ID
...
async def test_download(client, file_storage):
    url = await file_storage.download(FILE_ID, NAME)
...
async def test_download_no_file(client, file_storage):
    ...
    with pytest.raises(BaseAppException) as exc_info:
        await file_storage.download(FILE_ID, NAME)
